...
% Define bar width
bar_width = 0.1;  % Set the width of each bar to 0.1
...
for j = 1:length(images)
    ...
    for i = 1:length(filters)
        ...
        bar_value = psnr_values.(filters{i})(j);

        % Set x positions based on the image number and filter position
        x_pos = j + (i - (length(filters) / 2)) * bar_width;  % Adjust x_pos for each filter

        % Plot the bars with the corresponding color
        bar(x_pos, bar_value, 'FaceColor', colors(i, :), 'BarWidth', bar_width);
    end
end

% Customize x-axis ticks
xticks(1:15);  % Set x-axis ticks for each image number
xticklabels(arrayfun(@num2str, 1:15, 'UniformOutput', false));  % Only show numbers 1 to 15

% Labeling and title
xlabel('Image');
ylabel('PSNR (dB)');
title('PSNR Comparison (Bar Graph)');
legend(filters, 'Location', 'bestoutside');
grid on;
hold off;